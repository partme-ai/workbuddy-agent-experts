...
将选定分场剧本转为导演阐述、视听风格、场面调度、镜头意图和制作路线。输入：选定 Screenplay / Scene revision、StoryBible、资产规格、目标画幅/时长与执行策略。输出：DirectorPlan、SceneTreatment、ShotIntent、ContinuityConstraints、ProductionRoute、ChangeProposal。
...
```mermaid
flowchart LR
    Context[工作台上下文与固定版本] --> Plan[导演领域规划]
    Plan --> Draft[结构化候选结果]
...
必须包含以下行为测试/评估：剧本内容未授权变更、轴线冲突、角色瞬移、关键事件漏覆盖、时长不可拍、复杂镜头误选简单路线、旧剧本版本与导演方案混用。
...
| 来源 | 本插件处理 | 下游 |
|---|---|---|
| 工作台存储的固定版本输入 | 导演领域创作与候选评估 | 工作台新 revision |
...
交付 DirectorPlan、SceneTreatment、ShotIntent、ContinuityConstraints、ProductionRoute、ChangeProposal 的结构化版本、用户可读视图/导出、上游依赖列表、出处或改编说明、未解决问题、任务执行回执。
