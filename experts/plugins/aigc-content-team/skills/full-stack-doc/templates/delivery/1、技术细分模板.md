...
| :--- | :--- | :--- | :--- | :--- |
| V1.0.0 | 202x-xx-xx | {{OWNER}} | 初始版本 | 对应需求文档 V1.0 |

### 1.2 责任人
| 角色 | 姓名 | 职责 |
| :--- | :--- | :--- |
| 产品 | {{OWNER}} | 输出需求文档、验收 |
| 前端 | {{OWNER}} | 前端架构、页面开发、交互实现 |
| 后端 | {{OWNER}} | 数据库设计、接口开发、服务逻辑 |
| 测试 | {{OWNER}} | 输出测试用例、质量把控 |
...
| 模块 | 任务详情 | 预估工时(h) | 备注 |
| :--- | :--- | :--- | :--- |
| {{MODULE_NAME}} | {具体任务，如：订单列表页UI开发} | 4 | 依赖UI设计稿完成 |
| {{MODULE_NAME}} | {具体任务，如：倒计时逻辑调试} | 2 | 需联调后端时间戳接口 |
| **合计** | - | **{Total}** | |

### 7.2 后端工时
| 模块 | 任务详情 | 预估工时(h) | 备注 |
| :--- | :--- | :--- | :--- |
| {{MODULE_NAME}} | {具体任务，如：订单状态流转接口} | 6 | 需确认支付回调逻辑 |
| {{MODULE_NAME}} | {具体任务，如：数据库表结构变更} | 2 | 需DBA审核 |
| **合计** | - | **{Total}** | |

### 7.3 需求计划
> 本次需求由产品 `{{OWNER}}` 提出，测试 `{{OWNER}}` 测试，前端 `{{OWNER}}` 参与，后端 `{{OWNER}}` 参与。
