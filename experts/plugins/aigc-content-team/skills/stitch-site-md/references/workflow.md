...
- **Stitch Project ID**: Use a verified active ID if available; otherwise explicitly state that no remote project is linked.
...
Record permitted variations, explicit non-goals and the authorized iteration limit. An empty backlog is a stop condition, not permission to invent pages.
...
4. **Validate**: Verify that all six sections of the `SITE.md` template are present and properly formatted.
