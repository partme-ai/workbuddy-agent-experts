...
echo "Downloading Stitch asset (URL omitted to protect signed query parameters)"
...
STAGING_PATH=$(mktemp "${OUTPUT_PATH}.download.XXXXXX")
trap 'rm -f "$STAGING_PATH"' EXIT
if curl --proto '=https' --proto-redir '=https' -L -f --connect-timeout 10 --max-time 120 -o "$STAGING_PATH" "$DOWNLOAD_URL"; then
