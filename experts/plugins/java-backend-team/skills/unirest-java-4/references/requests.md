# Making Requests — Full Reference

Complete guide to building HTTP requests with Unirest-Java.
...
Unirest.get("http://localhost")
    ...
    .queryString(Map.of("droid", "R2D2", "beatle", "Ringo"))
    .asString();
...
Object body (serialized via ObjectMapper):
```java
Unirest.post("http://localhost")
...
The `PagedList` has methods for dealing with the results.
